...
$query_pesquisa = "SELECT v.*, u.nome, u.sobrenome, c.nome as 'nome_categoria' FROM video v INNER JOIN usuario u ON (v.video_idUser = u.id_user) INNER JOIN video_has_categoria vhc ON (v.id_video = vhc.id_video) INNER JOIN categoria c ON (c.id_categoria = vhc.id_categoria) WHERE v.titulo LIKE '%$pesquisa%' OR v.descricao LIKE '%$pesquisa%' OR 'nome_categoria' LIKE '%$pesquisa%' GROUP BY v.id_video ORDER BY v.data_postagem DESC";
